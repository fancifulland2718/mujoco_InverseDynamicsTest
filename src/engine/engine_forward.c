...
void mj_fwdVelocity(const mjModel* m, mjData* d) {
  ...
  // 分别计算柔性体、肌腱、执行器的速度，因为调用了不同的对象
  // 柔性体和肌腱都是Jacobi乘以关节速度，执行器是执行器力矩乘以关节速度
...
// 输入：(qpos, qvel, ctrl, act) 输出：(qfrc_actuator, actuator_force, act_dot)
void mj_fwdActuation(const mjModel* m, mjData* d) {
    ...
    for (int i = 0; i < nu; i++) {
        ...
        case mjGAIN_FIXED:              // 固定增益：参数为增益值
            gain = prm[0];
            break;

        case mjGAIN_AFFINE:             // 仿射增益：参数为[常数项, 位置系数, 速度系数]
            gain = prm[0] + prm[1] * d->actuator_length[i] + prm[2] * d->actuator_velocity[i];
            break;

        case mjGAIN_MUSCLE:             // 肌肉增益模型
        ...
        case mjBIAS_NONE:               // 无偏置
            bias = 0.0;
            break;

        case mjBIAS_AFFINE:             // 仿射偏置：参数为[常数项, 位置系数, 速度系数]
            bias = prm[0] + prm[1] * d->actuator_length[i] + prm[2] * d->actuator_velocity[i];
            break;

        case mjBIAS_MUSCLE:             // 肌肉被动力偏置
    ...
    if (m->nplugin) {
        ...
        for (int i = 0; i < m->nplugin; i++) {
            ...
            if (plugin->capabilityflags & mjPLUGIN_ACTUATOR) {
                if (!plugin->compute) {
                    mjERROR("插件槽位 %d 的compute函数指针为空", slot);
                }
    ...
    // 计算关节力矩：qfrc_actuator = actuator_moment^T * actuator_force
...
void mj_forwardSkip(const mjModel* m, mjData* d, int skipstage, int skipsensor) {
  ...
  // position-dependent
  // 在一般的正向动力学阶段，skipstage = 0 ，因此所有阶段都会进行
