...
// 将金字塔摩擦模型参数转换为接触力向量
// 说明: 该函数将摩擦锥的分解表示转换为标准的(法向力, 切向力1, 切向力2,...)形式
// 参数: force   - 输出力向量[法向力, 切向力1, 切向力2,...]，大小为dim
//       pyramid - 输入金字塔参数，大小为2*(dim-1)，可以取 0，4，6，10
//       mu      - 各向摩擦系数数组，大小为dim-1
//       dim     - 空间维度，取值1，3，4，6，对应 法向(1)、切向摩擦(2)、扭转摩擦(1)、滚动摩擦(2)

// 这个函数需要对比论文中的叙述，才能看清楚它的操作目的。
// 严格的摩擦锥约束应当是 || F_T || <= \mu || F_N || 
// 这里的操作是：首先把力分解为正部和负部之差，这两个部分都是非负的，这一步已经提前做好了
//             然后考虑所有正负分量的总和，作为法向力，以控制切向力范围
//             计算实际的摩擦，再后续过程中使用
void mju_decodePyramid(mjtNum* force, const mjtNum* pyramid, const mjtNum* mu, int dim) {
    // 处理无摩擦接触的特殊情况
    if (dim == 1) {
        force[0] = pyramid[0];  // 直接取法向力
        return;
    }

    // 计算法向力: 金字塔参数所有元素之和
    // 原理: 金字塔模型将法向力分解为各方向正负分量的和
    force[0] = 0;
    for (int i = 0; i < 2 * (dim - 1); i++) {
        force[0] += pyramid[i];  // 累加所有金字塔参数(每个摩擦方向有两个分量)
    }

    // 计算各切向力: (正分量 - 负分量) * 对应摩擦系数
    // 索引说明: 每个摩擦方向对应两个连续元素，如方向0对应pyramid[0]和pyramid[1]
    for (int i = 0; i < dim - 1; i++) {
        // 切向力 = (正方向分量 - 负方向分量) * 摩擦系数
