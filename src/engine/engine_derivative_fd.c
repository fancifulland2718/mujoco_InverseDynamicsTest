...
// 使用有限差分法计算逆动力学的雅可比矩阵（力、传感器数据、质量矩阵关于状态变量的导数）
// 参数说明：
//   m: 模型对象指针，包含系统模型信息
//   d: 数据对象指针，包含当前状态和计算结果
//   eps: 有限差分步长
//   flg_actuation: 标志位，是否从逆动力学力中减去执行器力
//   后续参数均为输出矩阵指针（按列存储，转置了控制理论惯例的行列方向）：
//     DfDq: 力关于位置的雅可比矩阵 (nv x nv)
//     DfDv: 力关于速度的雅可比矩阵 (nv x nv)
//     DfDa: 力关于加速度的雅可比矩阵 (nv x nv)
//     DsDq: 传感器数据关于位置的雅可比矩阵 (nv x nsensordata)
//     DsDv: 传感器数据关于速度的雅可比矩阵 (nv x nsensordata)
//     DsDa: 传感器数据关于加速度的雅可比矩阵 (nv x nsensordata)
//     DmDq: 质量矩阵关于位置的雅可比矩阵 (nv x nM)
void mjd_inverseFD(const mjModel* m, mjData* d, mjtNum eps, mjtByte flg_actuation,
    ...
    // 标记是否跳过传感器计算（当所有传感器导数指针均为空时）
    int skipsensor = !DsDq && !DsDv && !DsDa;

    // 在栈上分配临时内存空间
    mj_markStack(d);
    mjtNum* pos = mjSTACKALLOC(d, nq, mjtNum);                      // 当前位置备份
    mjtNum* force = mjSTACKALLOC(d, nv, mjtNum);                      // 基准力向量
    mjtNum* force_plus = mjSTACKALLOC(d, nv, mjtNum);                      // 扰动后的力向量
    mjtNum* sensor = skipsensor ? NULL : mjSTACKALLOC(d, ns, mjtNum);  // 基准传感器数据
    mjtNum* mass = DmDq ? mjSTACKALLOC(d, nM, mjtNum) : NULL;        // 基准质量矩阵
    ...
    if (sensor) mju_copy(sensor, d->sensordata, ns);  // 保存基准传感器数据
    if (mass) mju_copy(mass, d->qM, nM);              // 保存基准质量矩阵
    ...
    if (DfDv || DsDv) {
        for (int i = 0; i < nv; i++) {
            ...
            // 计算传感器数据关于速度的有限差分（第i行）
            if (DsDv) diff(DsDv + i * ns, sensor, d->sensordata, eps, ns);
        }
    }

    // 计算位置导数 (DfDq, DsDq, DmDq)
    if (DfDq || DsDq || DmDq) {
        mjtNum* dpos = mjSTACKALLOC(d, nv, mjtNum);  // 位置扰动向量
        for (int i = 0; i < nv; i++) {
            // 生成单位扰动向量
            mju_zero(dpos, nv);
            dpos[i] = 1.0;
