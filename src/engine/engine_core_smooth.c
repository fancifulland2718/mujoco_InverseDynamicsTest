...
void mj_comVel(const mjModel* m, mjData* d) {
  ...
  for (int i = 1; i < nbody; i++) {
      ...
      // 计算：cvel = 父cvel + cdof * qvel，cdofdot = cvel x cdof
      int dofnum = m->body_dofnum[i];        // 当前刚体的自由度数量
      mjtNum cdofdot[36];                    // 临时存储自由度导数
      for (int j = 0; j < dofnum; j++) {
          mjtNum tmp[6];                       // 临时速度计算缓冲区

          // 根据关节类型进行不同计算
          switch ((mjtJoint)m->jnt_type[m->dof_jntid[bda + j]]) {
          case mjJNT_FREE:                     // 自由关节（6自由度）
              // 前3个平移自由度导数置零
              mju_zero(cdofdot, 18);
              ...
              // 继续处理后续3个旋转自由度（使用fallthrough特性）
              j += 3;
              mjFALLTHROUGH;

          case mjJNT_BALL:                     // 球关节（3旋转自由度）
              // 使用父速度计算所有3个旋转自由度的导数
              for (int k = 0; k < 3; k++) {
          ...
          default:                             // 其他关节类型（滑动/铰链等）
              // 使用当前速度计算自由度导数（cdof与自身叉乘为零，可用旧值优化）
      ...
      // 将计算结果写入数据缓存
      mju_copy(d->cvel + 6 * i, cvel, 6);                      // 存储刚体速度
      mju_copy(d->cdof_dot + 6 * bda, cdofdot, 6 * m->body_dofnum[i]);  // 存储自由度导数
  }
}
...
// RNE算法实现：计算广义惯性力 M(qpos)*qacc + 非线性力 C(qpos,qvel)
// 参数: flg_acc - 0表示忽略惯性项(即不计算M(qpos)*qacc)
//       result - 输出结果数组，存储计算后的广义力
void mj_rne(const mjModel* m, mjData* d, int flg_acc, mjtNum* result) {
    int nbody = m->nbody, nv = m->nv;
    mjtNum tmp[6], tmp1[6];
    mj_markStack(d);  // 标记当前栈帧，用于内存分配追踪
    // 分配临时内存：物体级联加速度和物体力向量
    // 旋转轴等信息都是事先规定好的，这里不必考虑，只要考虑平动和转动的力
    mjtNum* loc_cacc = mjSTACKALLOC(d, m->nbody * 6, mjtNum);      // 每个物体6维加速度
    mjtNum* loc_cfrc_body = mjSTACKALLOC(d, m->nbody * 6, mjtNum); // 每个物体6维力

    // 初始化世界坐标系的加速度为负重力(惯性系下的虚拟加速度)
    mju_zero(loc_cacc, 6);
    if (!mjDISABLED(mjDSBL_GRAVITY)) {  // 如果启用了重力
        mju_scl3(loc_cacc + 3, m->opt.gravity, -1);  // 设置线性加速度为-重力，位置偏移3表示线性分量
    }

    // 下面前后向遍历的计算过程是RNE算法的精髓
    // 前向遍历：计算每个物体的级联加速度和组合惯性力
    for (int i = 1; i < nbody; i++) {  // 从1开始跳过世界体
        int bda = m->body_dofadr[i];  // 当前物体自由度的起始索引

        // 计算cacc: 父级加速度 + cdof_dot * qvel (科氏力项)，cacc 基于质心的加速度
        // cdof_dot 基于质心的自由度运动轴的导数 , cdof 是 nv*6 尺寸
        mju_mulDofVec(tmp, d->cdof_dot + 6 * bda, d->qvel + bda, m->body_dofnum[i]);
        mju_add(loc_cacc + 6 * i, loc_cacc + 6 * m->body_parentid[i], tmp, 6);

        // 若启用加速度项，添加cdof * qacc (惯性力项)
        if (flg_acc) {
            mju_mulDofVec(tmp, d->cdof + 6 * bda, d->qacc + bda, m->body_dofnum[i]);
            mju_addTo(loc_cacc + 6 * i, tmp, 6);
        }

        // 计算物体所受内力: 惯性矩阵*加速度 + 速度项产生的科氏力
        mju_mulInertVec(loc_cfrc_body + 6 * i, d->cinert + 10 * i, loc_cacc + 6 * i);  // cinert是10元素的压缩惯性矩阵
        mju_mulInertVec(tmp, d->cinert + 10 * i, d->cvel + 6 * i);  // 计算惯性速度项
        mju_crossForce(tmp1, d->cvel + 6 * i, tmp);  // 计算速度叉乘项
        mju_addTo(loc_cfrc_body + 6 * i, tmp1, 6);   // 组合得到总内力
    }

    // 清除世界体的内力(世界体不参与动力学计算)
    mju_zero(loc_cfrc_body, 6);

    // 后向遍历：将子物体的内力累加到父物体
    for (int i = nbody - 1; i > 0; i--)
        if (m->body_parentid[i]) {  // 跳过没有父物体的世界体
            mju_addTo(loc_cfrc_body + 6 * m->body_parentid[i], loc_cfrc_body + 6 * i, 6);
        }

    // 将物体内力投影到关节空间，得到广义力
    for (int i = 0; i < nv; i++) {
        // 使用cdof矩阵将6D力向量投影到自由度空间
        result[i] = mju_dot(d->cdof + 6 * i, loc_cfrc_body + 6 * m->dof_bodyid[i], 6);
    }

    mj_freeStack(d);  // 释放栈分配的内存
}

// 后处理约束力计算：计算完整的cacc, cfrc_ext, cfrc_int
// 说明：在约束计算之后调用，处理接触力、外力等综合效果
void mj_rnePostConstraint(const mjModel* m, mjData* d) {
    int nbody = m->nbody;
    mjtNum cfrc_com[6], cfrc[6], lfrc[6];
    mjContact* con;

    // 初始化加速度，设置世界坐标系加速度为负重力
    mju_zero(d->cacc, 6);
    if (!mjDISABLED(mjDSBL_GRAVITY)) {
        mju_scl3(d->cacc + 3, m->opt.gravity, -1);
    }

    // 处理外部施加的力(perturbation forces)
    mju_zero(d->cfrc_ext, 6 * nbody);
    for (int i = 1; i < nbody; i++)
        if (!mju_isZero(d->xfrc_applied + 6 * i, 6)) {  // 检查是否存在外力
            // 将力/扭矩转换为(扭矩,力)顺序，并转换到质心坐标系
            mju_copy3(cfrc, d->xfrc_applied + 6 * i + 3);    // 提取扭矩
            mju_copy3(cfrc + 3, d->xfrc_applied + 6 * i);    // 提取力
            mju_transformSpatial(cfrc_com, cfrc, 1,
                d->subtree_com + 3 * m->body_rootid[i],  // 子树质心
                d->xipos + 3 * i, 0);  // 物体局部坐标系原点
            mju_addTo(d->cfrc_ext + 6 * i, cfrc_com, 6);  // 累加到外部力
        }

    // 处理接触力
    int ncon = d->ncon;
    for (int i = 0; i < ncon; i++)
        if (d->contact[i].efc_address >= 0) {  // 仅处理有效的接触
            con = d->contact + i;
            if (con->geom[0] < 0 || con->geom[1] < 0) continue;  // 跳过柔性体接触

            mj_contactForce(m, d, i, lfrc);  // 获取局部接触力(扭矩在前，力在后)

            // 转换接触力到世界坐标系
            mju_mulMatTVec3(cfrc, con->frame, lfrc + 3);  // 旋转力分量
            mju_mulMatTVec3(cfrc + 3, con->frame, lfrc);   // 旋转扭矩分量

            // 对两个接触物体应用相反的力
            int k;
            if ((k = m->geom_bodyid[con->geom[0]])) {  // 第一个物体
                mju_transformSpatial(cfrc_com, cfrc, 1,
                    d->subtree_com + 3 * m->body_rootid[k], // 子树质心
                    con->pos, 0);  // 接触点位置
                mju_subFrom(d->cfrc_ext + 6 * k, cfrc_com, 6);  // 反向施加
            }
            if ((k = m->geom_bodyid[con->geom[1]])) {  // 第二个物体
                mju_transformSpatial(cfrc_com, cfrc, 1,
                    d->subtree_com + 3 * m->body_rootid[k],
                    con->pos, 0);
                mju_addTo(d->cfrc_ext + 6 * k, cfrc_com, 6);
            }
        }

    // 处理连接约束(connect/weld/flex等)
    int i = 0, ne = d->ne;
    while (i < ne) {
        if (d->efc_type[i] != mjCNSTR_EQUALITY)
            mjERROR("row %d of efc is not an equality constraint", i);
        ...
        case mjEQ_WELD: {
            ...
            if (k) {
                offset = body_semantic ? eq_data + 3 * (m->eq_type[id] == mjEQ_WELD) :
                    m->site_pos + 3 * obj1;
                mj_local2Global(d, pos, 0, offset, 0, k, 0);  // 局部->全局坐标转换
                mju_transformSpatial(cfrc_com, cfrc, 1,
                    d->subtree_com + 3 * m->body_rootid[k], pos, 0);
                mju_addTo(d->cfrc_ext + 6 * k, cfrc_com, 6);  // 施加约束力
            }
        ...
        case mjEQ_FLEX: {
            // 处理柔性体的非刚性边约束
            int k = m->eq_obj1id[id];
            int flex_edgeadr = m->flex_edgeadr[k];
            int flex_edgenum = m->flex_edgenum[k];
            for (int e = flex_edgeadr; e < flex_edgeadr + flex_edgenum; e++) {
                if (!m->flexedge_rigid[e]) i++;  // 非刚性边对应约束行
            }
            break;
        }

        default:
            mjERROR("unknown constraint type type %d", m->eq_type[id]);
        }
    }
