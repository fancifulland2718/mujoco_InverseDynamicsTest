...
// 标记当前线程的栈帧信息，用于内存管理或调试目的
// 说明: 该函数根据是否使用线程池，获取当前线程的栈信息并调用内部标记函数
// 参数: d - 指向mjData结构体的指针，包含模型数据和线程池信息
#ifndef ADDRESS_SANITIZER  // 条件编译：若非地址消毒器模式
void mj_markStack(mjData* d)  // 正常模式函数名
#else
void mj__markStack(mjData* d)  // 地址消毒器模式使用不同函数名避免冲突
#endif
{
    // 场景1: 无线程池时的单线程处理
    if (!d->threadpool) {
        // 获取当前线程的栈空间信息(栈顶和栈底地址)
        mjStackInfo stack_info = get_stack_info_from_data(d);
        ...
        // 记录栈指针位置：计算栈深度和存储基地址
        d->pstack = stack_info.bottom - stack_info.top;  // 栈深度 = 栈底地址 - 栈顶地址
        d->pbase = stack_info.stack_base;  // 存储当前栈的基地址
        return;
    }
