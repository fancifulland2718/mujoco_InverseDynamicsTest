...
// 更新约束力并计算成本，针对特定的约束岛（island）
void mj_constraintUpdate_island(const mjModel* m, mjData* d, const mjtNum* jar,
    mjtNum cost[1], int flg_coneHessian, int island) {
    int ne = d->ne, nf = d->nf;  // 等式约束和摩擦约束的数量
    const mjtNum* D = d->efc_D, * R = d->efc_R, * floss = d->efc_frictionloss;  // 约束参数指针
    mjtNum* force = d->efc_force;  // 约束力数组指针
    mjtNum s = 0;  // 成本累加器
    ...
    // 无约束情况处理：清零约束力并返回
    if (!nefc) {
        mju_zero(d->qfrc_constraint, m->nv);  // 清零广义约束力
        if (cost) *cost = 0;  // 成本置零
        return;
    }

    // 第一步：计算无约束情况下的efc_force（基于雅可比乘积jar）
    for (int c = 0; c < nefc; c++) {
        int i = efcind ? efcind[c] : c;  // 选择当前约束的全局索引
        force[i] = -D[i] * jar[c];  // 力 = -阻尼系数 * 雅可比乘积
    }

    // 第二步：遍历所有约束，根据类型更新状态和计算成本
    for (int c = 0; c < nefc; c++) {
        int i = efcind ? efcind[c] : c;  // 当前约束的全局索引

        // ==== 处理等式约束 ====
        if (i < ne) {
            if (cost) s += 0.5 * D[i] * jar[c] * jar[c];  // 二次成本累加
            d->efc_state[i] = mjCNSTRSTATE_QUADRATIC;  // 标记为二次状态
            continue;
        }
        ...
        if (i < ne + nf) {
            ...
            if (jar[c] <= -R[i] * floss[i]) {
                if (cost) s += -0.5 * R[i] * floss[i] * floss[i] - floss[i] * jar[c];
                force[i] = floss[i];  // 力设为摩擦损失
                d->efc_state[i] = mjCNSTRSTATE_LINEARNEG;
            }
            // 线性正区域（摩擦力达到上限）
            else if (jar[c] >= R[i] * floss[i]) {
                if (cost) s += -0.5 * R[i] * floss[i] * floss[i] + floss[i] * jar[c];
                force[i] = -floss[i];  // 力设为负摩擦损失
                d->efc_state[i] = mjCNSTRSTATE_LINEARPOS;
            }
            // 二次区域（中间状态）
            else {
                if (cost) s += 0.5 * D[i] * jar[c] * jar[c];
        ...
        // 椭圆锥约束处理
            ...
            mjContact* con = d->contact + d->efc_id[i];  // 获取接触信息
            mjtNum mu = con->mu, * friction = con->friction;  // 摩擦系数和摩擦方向
            int dim = con->dim;  // 接触维度（通常3D为3）

            // 映射到双锥空间
            mjtNum U[6];
            U[0] = jar[c] * mu;  // 法向分量
            for (int j = 1; j < dim; j++)
                U[j] = jar[c + j] * friction[j - 1];  // 切向分量

            mjtNum N = U[0];  // 法向力
            mjtNum T = mju_norm(U + 1, dim - 1);  // 切向力的范数

            // 顶部区域（无滑动）
            if (N >= mu * T || (T <= 0 && N >= 0)) {
                mju_zero(force + i, dim);  // 力清零
                d->efc_state[i] = mjCNSTRSTATE_SATISFIED;
            }
            // 底部区域（完全滑动）
                ...
                if (cost) {
                    for (int j = 0; j < dim; j++)
                        s += 0.5 * D[i + j] * jar[c + j] * jar[c + j];  // 各维度成本累加
                }
                d->efc_state[i] = mjCNSTRSTATE_QUADRATIC;
            }
            // 中间区域（部分滑动）
            else {
                mjtNum Dm = D[i] / (mu * mu * (1 + mu * mu));  // 阻尼系数修正
                mjtNum NmT = N - mu * T;  // 法向与切向组合量

                if (cost) s += 0.5 * Dm * NmT * NmT;  // 锥形成本计算

                // 计算锥形约束力
                force[i] = -Dm * NmT * mu;  // 法向分量
                for (int j = 1; j < dim; j++)
                    force[i + j] = -force[i] / T * U[j] * friction[j - 1];  // 切向分量

                d->efc_state[i] = mjCNSTRSTATE_CONE;  // 标记为锥形状态

                // 计算锥形Hessian矩阵（如果需要）
                if (flg_coneHessian) {
                    mjtNum* H = d->contact[d->efc_id[i]].H;  // Hessian存储指针

                    // 构造Hessian矩阵（描述二阶导数关系）
                    H[0] = 1;  // 第一行第一个元素
                    mjtNum scl = -mu / T;
                    for (int j = 1; j < dim; j++)
                        H[j] = scl * U[j];  // 第一行其他元素
    ...
    // 第三步：将efc_force映射到广义坐标空间
    mj_mulJacTVec_island(m, d, d->qfrc_constraint, d->efc_force,
        island, 1, 1);
